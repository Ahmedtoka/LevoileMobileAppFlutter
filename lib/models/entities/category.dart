...
class Category {
  ...
  Category.fromJsonShopify(Map parsedJson) {
    ...
    try {
      ...
      // Track whether the collection has any product so empty ones can be
      // hidden (Shopify Storefront has no direct product count).
      final productEdges = parsedJson['products']?['edges'];
      totalProduct = (productEdges is List) ? productEdges.length : null;
      final image = parsedJson['image'];
      if (image != null) {
        this.image = image['url'].toString();
      } else if (productEdges is List &&
          productEdges.isNotEmpty &&
          productEdges.first['node']?['featuredImage']?['url'] != null) {
        // Fallback: use the first product's photo when the collection itself
        // has no image set in Shopify, so category circles aren't empty.
        this.image = productEdges.first['node']['featuredImage']['url']
            .toString();
      } else {
        this.image = kDefaultImage;
      }
