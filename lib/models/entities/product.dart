...
class Product {
  ...
  Product.fromShopify(Map json) : id = json['id'] {
    try {
      ...
      // Only treat as on-sale when compareAtPrice is a real, higher price.
      // Shopify can return compareAtPrice = 0.0 for non-discounted items,
      // which previously caused a struck-through 0.00 and a wrong % badge.
      final compareAtNum = double.tryParse('${compareAtPrice ?? ''}') ?? 0.0;
      final priceNum = double.tryParse('${price ?? ''}') ?? 0.0;
      final isOnSale = compareAtNum > priceNum && compareAtNum > 0.0;
      onSale = isOnSale;
      regularPrice = isOnSale ? compareAtPrice : price;
      type = '';
      salePrice = price;
