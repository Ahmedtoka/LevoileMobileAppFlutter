...
import '../../screens/checkout/webview_checkout_screen.dart';
...
class ShopifyWidget extends BaseFrameworks
        ...
        Row(
          children: <Widget>[
            ...
            name == 'color'
                ...
                : Expanded(child: Text(option ?? '', textAlign: TextAlign.end)),
          ],
        ),
      );
  ...
  Future<bool> changePassword({
    ...
    try {
      ...
      final updateParams = {'password': newPassword};
      ...
      await shopifyService.updateUserInfo(updateParams, tokenResult.token);
  ...
  Future<void> applyCoupon(
    ...
    try {
      ...
      if (cartDataShopify == null) {
        error?.call('Cannot apply coupon for now. Please try again later.');
        return;
      }
  ...
  Future<void> removeCoupon(context) async {
    ...
    try {
      final cartRemovedCoupon = await shopifyService.removeCouponWithCartId(
        cartDataShopify.id,
      );
  ...
  Map<dynamic, dynamic> getPaymentUrl(context) {
    return {
      'headers': {},
      'url': Provider.of<CartModel>(
        context,
        listen: false,
      ).cartDataShopify?.checkoutUrl,
    };
  }
  ...
  Future<void> doCheckout(
    ...
    try {
      final cartDataShopify = await shopifyService.createCart(
        cartModel: cartModel,
      );
      if (cartDataShopify == null) {
        ...
        if (orderNum != null && !kIsWeb) {
          ...
          if (kPaymentConfig.showNativeCheckoutSuccessScreenForWebview) {
            await Navigator.push(
              ...
              MaterialPageRoute(
                builder: (context) =>
                    WebviewCheckoutSuccessScreen(order: successOrder),
              ),
            );
          }
        }
  ...
  void placeOrder(
  ...
  }) async {
    {
      ...
      if (note != null) {
        await shopifyService.updateCartNote(cartId: cartId, note: note);
      }
                ...
                if (user != null && (user.cookie?.isNotEmpty ?? false)) {
                  ...
                  Analytics.triggerPurchased(
                    Order(
                      number: orderNum,
                      total:
                          cartModel.cartDataShopify?.cost.totalAmount.amount ??
                          0,
                      id: '',
                    ),
                    context,
  ...
  Future<bool> updateCartBuyerIdentity({
    ...
    if (cartData.buyerIdentity.customer == null) {
      final result = await shopifyService.updateCartBuyerIdentity(
        cartId: cartData.id,
        buyerIdentity: {},
      );
      if (result == null) {
  ...
  Future<bool> cartDeliveryAddressUpdate({
    ...
    if (address.id != null && address.id!.contains('MailingAddress')) {
      final cartDataShopify = await shopifyService
          .updateCartDeliveryAddressesWithId(
