...
class ShopifyService extends BaseServices {
  ShopifyService({
    ...
    _reviewService = ReviewService.create();
  ...
  static GraphQLClient getClient({
    ...
    return GraphQLClient(
      ...
      // Default is 5s which is too short for heavier queries (e.g. collection
      // lists). Give Shopify more breathing room before timing out.
  ...
  Future<List<Category>> getCategoriesByCursor({
    ...
    try {
      ...
      // Smaller page keeps the collections+product-count query fast enough to
      // stay under the request timeout; remaining pages load via cursor.
      ...
      if (data != null) {
        ...
        for (var item in collections?['edges']) {
          ...
          final cat = Category.fromJsonShopify(category);
          // Skip empty collections (e.g. "Skirt" with no products) when the
          // HideEmptyCategories option is enabled.
          if (kAdvanceConfig.hideEmptyCategories &&
              (cat.totalProduct ?? 0) <= 0) {
            continue;
          }
          list.add(cat);
        }
    ...
    try {
      ...
      final options = QueryOptions(
        ...
        variables: <String, dynamic>{
          'nRepositories': nRepositories,
          'query': '$name',
          if (cursor != null) 'cursor': cursor else 'cursor': ?page,
          'pageSize': pageSize,
  ...
  Future<User> createUser({
    ...
    try {
      ...
      final listError = List.from(
        result.data?['customerCreate']?['userErrors'] ?? [],
      );
      if (listError.isNotEmpty) {
        // Existing (often guest) customer: the email is already on file but
        // has no password. Instead of a dead-end error, trigger Shopify's
        // "set your password" email so they can activate and sign in.
        final isTaken = listError.any((e) {
          final code = '${e['code']}'.toUpperCase();
          final msg = '${e['message']}'.toLowerCase();
          return code.contains('TAKEN') ||
              msg.contains('already') ||
              msg.contains('taken');
        });
        if (isTaken && (email?.isNotEmpty ?? false)) {
          try {
            await submitForgotPassword(data: {'email': email});
          } catch (_) {}
          throw ('هذا البريد لديه حساب بالفعل. أرسلنا لك إيميل لتعيين كلمة المرور — عيّنها ثم سجّل الدخول.\nThis email already has an account. We emailed you a link to set your password — set it, then log in.');
        }
      ...
      final tokenResult = await createAccessToken(
        email: email,
        password: password,
      );
  ...
  Future<User?> getUserInfo(cookie, {DateTime? tokenExpiresAt}) async {
    try {
      ...
      if (isCustomerAccountEnabled) {
        return await customerAccountService!.getUserInfo(
  ...
  Future<Map<String, dynamic>> updateUserInfo(
    ...
    try {
      ...
      final error = errors?.firstWhereOrNull(
        (element) => element['message'] != null,
      );
      if (error != null) {
  ...
  Future<User?> login({username, password}) async {
    try {
      ...
      final result = await createAccessToken(
        email: username,
        password: password,
      );
  ...
  Future<PaymentSettings> getPaymentSettings() async {
    try {
      ...
      final options = QueryOptions(
        ...
        variables: const <String, dynamic>{'nRepositories': nRepositories},
      );
  ...
  Future<PagingResponse<Order>> getMyOrders({
    ...
    try {
      ...
      return PagingResponse(cursor: lastCursor, data: list);
    } catch (e) {
  ...
  Future<Product?> getProductByPermalink(String productPermalink) async {
    final handle = productPermalink.substring(
      productPermalink.lastIndexOf('/') + 1,
    );
  ...
  @override
  Future<Category?> getProductCategoryById({required String categoryId}) async {
  ...
  Future<CartDataShopify?> cartPaymentUpdate({
    ...
    try {
      final options = MutationOptions(
        ...
        variables: <String, dynamic>{'id': cartId, 'payment': paymentData},
      );
      final result = await _connector.mutate(options);
  ...
  Future<PagingResponse<Review>> getReviews(
    ...
    int page = 1,
    int perPage = 10,
  }) => _reviewService.getReviews(productId, page: page, perPage: perPage);

  @override
  Future<RatingCount?>? getProductRatingCount(String productId) async {
    return _reviewService.getProductRatingCount(productId);
  }

  @override
  Future? createReview(ReviewPayload payload) {
    return _reviewService.createReview(payload);
  }
  ...
  Future<PagingResponse<Review>> getListReviewByUserEmail(
    ...
    return _reviewService.getListReviewByUserEmail(
      email,
      page: page,
      perPage: perPage,
      status: status,
  ...
  Future<List<Currency>?> getAvailableCurrencies() async {
    try {
      ...
      final availableCountries = List.from(
        result.data?['localization']?['availableCountries'] ?? [],
      );
      ...
      for (final group in deliveryGroups) {
        for (final option in group.deliveryOptions) {
          final optionWithGroupId = option.toJson()
            ..addAll({'deliveryGroupId': group.id});
  ...
  Future<CartDataShopify> getCart({required String cartId}) async {
    try {
      ...
      printLog('::::getCart shopify error');
      printError(e, trace);
      rethrow;
    }
  }

  /// Per-order platform label written to the Shopify cart attributes, so each
  /// order shows whether it came from Android or iOS (under "Additional
  /// details" in the Shopify admin).
  String get _appSourceLabel {
    if (kIsWeb) return 'Le Voile App – Web';
    switch (defaultTargetPlatform) {
      case TargetPlatform.iOS:
        return 'Le Voile App – iOS';
      case TargetPlatform.android:
        return 'Le Voile App – Android';
      default:
        return 'Le Voile App';
    }
  }
  ...
  Future<CartDataShopify?> createCart({
    required CartModelShopify cartModel,
  }) async {
    try {
      var lineItems = [];
      ...
      final options = MutationOptions(
        ...
        variables: {
          'input': {
            'lines': lineItems,
            // Tag every order with the platform it came from. Shows in the
            // Shopify order under "Additional details" (Source = Android/iOS).
            'attributes': [
              {'key': 'Source', 'value': _appSourceLabel},
            ],
            'buyerIdentity': {
              'countryCode': countryCode,
              // Prefill the Shopify checkout with the customer's details so
              // they don't have to retype email/phone every time.
              if (cartModel.user?.email?.isNotEmpty ?? false)
                'email': cartModel.user!.email,
              if ((cartModel.user?.phoneNumber?.startsWith('+') ?? false))
                'phone': cartModel.user!.phoneNumber,
  ...
  Future<CartDataShopify?> removeCouponWithCartId(String cartId) async {
    try {
      ...
      final options = MutationOptions(
        document: gql(ShopifyQuery.cartDiscountCodesUpdate),
        variables: {'cartId': cartId, 'discountCodes': const []},
      );
  ...
  Future<CartDataShopify> updateCartNote({
    ...
    final options = MutationOptions(
      ...
      variables: <String, dynamic>{'cartId': cartId, 'note': note},
    );
          ...
          'addresses': [
            {
              ...
              'address': {'deliveryAddress': address.toShopifyJson()},
            },
          ],
        },
      );
  ...
  Future<CartSubmitResult> cartSubmitForCompletion({
    ...
    try {
      ...
      final options = MutationOptions(
        ...
        fetchPolicy: FetchPolicy.noCache,
        variables: {'cartId': cartId, 'attemptToken': Uuid().generateV4()},
      );
      ...
      return CartSubmitResult(
        status: CartSubmitStatus.error,
        errors: [CartSubmitError(code: 'UNKNOWN_ERROR', message: e.toString())],
      );
    }
  }
  ...
  Future<CartTax?>? getTaxes(CartModel cartModel, String? token) async {
    ...
    return CartTax(
      items: [Tax(title: 'Tax', amount: taxes)],
      total: taxes,
