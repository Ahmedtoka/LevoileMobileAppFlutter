...
class ShopifyCustomerAccountService {
  ...
  ShopifyCustomerAccountService({required this.config, required this.shopDomain})
    : _httpLink = HttpLink(
        'https://shopify.com/${config.shopId}/account/customer/api/$apiVersion/graphql',
      );

  AuthLink get _authLink =>
      AuthLink(getToken: () async => await getAccessToken());

  GraphQLClient get _client =>
      GraphQLClient(cache: GraphQLCache(), link: _authLink.concat(_httpLink));
  ...
  Future<Map<String, dynamic>> updateUserInfo(
    ...
    try {
      ...
      final user = User.fromShopifyCustomerAccount(
        userData,
  ...
  Future<String?> deleteCustomerAddress({required String addressId}) async {
    try {
      ...
      final options = MutationOptions(
        ...
        fetchPolicy: FetchPolicy.noCache,
        queryRequestTimeout: _timeOut,
        variables: <String, dynamic>{'addressId': addressId},
      );
