...
class _ShopifyChangePasswordScreenState
  ...
  void _validatePasswordTips(String password) {
    setState(() {
      ...
      _hasNumbersSpecialChars = RegExp(
        r'(?=.*\d)(?=.*[!@#$%^&*(),.?":{}|<>])',
      ).hasMatch(password);
    });
  }
  ...
  void _onChangePasswordPressed() {
    ...
    try {
      ...
      if (currentUser == null) {
        throw Exception(
          'User not found',
        ); // This is internal error, no need to localize
      }
...
class _RequirementItemWidget extends StatelessWidget {
  ...
  const _RequirementItemWidget({required this.text, required this.isSatisfied});
        ...
        children: [
          AnimatedSwitcher(
            ...
            child: isSatisfied
                ...
                : const SizedBox(key: ValueKey('empty'), width: 16, height: 16),
          ),
