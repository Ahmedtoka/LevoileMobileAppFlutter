...
class _SettingNotificationWidgetState extends State<SettingNotificationWidget>
  ...
  void onChangeLifecycleState(AppLifecycleState lifecycleState) async {
    if (LifecycleEventHandler.isPaused) {
      ...
      // The app may have been disposed while awaiting; bail out to avoid
      // reading context on an unmounted widget.
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    // Le Voile: "Notification Messages" list removed — only the toggle remains.
    const messageWidget = SizedBox.shrink();
