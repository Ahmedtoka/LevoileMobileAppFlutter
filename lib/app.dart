...
class AppState extends State<App>
  ...
  /// Throttles remote-config refetch when the app returns to the foreground.
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.resumed) {
      // Presence heartbeat for the dashboard "online now" count.
      CouponService.instance.track('heartbeat');
      final now = DateTime.now();
      if (_lastConfigFetch == null ||
          now.difference(_lastConfigFetch!).inSeconds > 20) {
        _lastConfigFetch = now;
        // Pull the latest layout from the Le Voile dashboard on resume.
        _app?.loadAppConfig();
      }
    }
  }
  ...
  void initState() {
    ...
    /// Register this device with the Le Voile dashboard for push + reports.
    _registerDeviceToken();

    /// Claim the welcome coupon for this device. The popup itself is shown
    /// from the Home screen (so it appears after the home loads).
    CouponService.instance.init();

    /// Prefetch dashboard-controlled announcement popups (text / image).
    PopupService.instance.fetch();

    super.initState();
  }

  /// Base URL of the Le Voile dashboard API, derived from the remote
  /// appConfig URL (e.g. http://host/api/v1/config/en -> http://host/api/v1).
  String? get _dashboardApiBase {
    ...
    try {
      ...
      await http.post(
        ...
        body: jsonEncode({
          ...
          'phone': CouponService.instance.accountPhone,
        }),
      );
      debugPrint('🎟️[Device] registered token + device_id='
          '${CouponService.instance.deviceId}');
    } catch (e, trace) {
      printError(e, trace);
    }
  }
  ...
  void onMessage(FStoreNotificationItem notification) {
    ...
    // Le Voile dashboard published a layout change -> reload config instantly.
    if (notification.additionalData?['type'] == 'config_update') {
      _app?.loadAppConfig();
    }
  }
