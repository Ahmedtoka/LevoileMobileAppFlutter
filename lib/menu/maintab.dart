...
import '../screens/custom/force_update_dialog.dart';
...
class MainTabsState extends CustomOverlayState<MainTabs>
  ...
  /// Le Voile OTA: ask the dashboard if a newer build exists. If so, show the
  /// (blocking) update popup over everything once the home is ready.
  void _checkForceUpdate() async {
    final info = await AppUpdateService.instance.check();
    if (info == null || !info.updateAvailable) return;
    if (!mounted) return;
    await ForceUpdateDialog.show(context, info);
  }
  ...
  void _initListenEvent(context) {
    ...
    _subLoadedAppConfig = eventBus.on<EventLoadedAppConfig>().listen((
      ...
      // Refetch popups so frequency / content changes apply on publish.
      PopupService.instance.fetch(force: true);

      // Only rebuild the tab structure when the TAB BAR itself changed.
      // Rebuilding on every config reload (e.g. on app resume) was wiping the
      // navigation stack and bouncing the user back to Home. Home/content
      // changes still refresh via their own Selectors without re-init.
      final newSig = _tabSignature();
      if (!isInitialized || newSig != _lastTabSignature) {
        _initTabData(context);
      }
    });
  }

  /// A lightweight fingerprint of the bottom tab bar (which tabs, in order).
  String _tabSignature() {
    try {
      final tabs =
          Provider.of<AppModel>(context, listen: false).appConfig?.tabBar;
      if (tabs == null) return '';
      return tabs.map((t) => '${t.layout}:${t.isFullscreen}').join('|');
    } catch (_) {
      return '';
    }
  }

  String? _lastTabSignature;
  ...
  Future<bool> _exitApp() async {
    ...
    if (exitAppType.isSystemDefault) {
      ...
      final isAllowExit = await showDialog<bool>(
        context: context,
        builder: (ctx) => Dialog(
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Thank you for visiting Le Voile.\n'
                  'Your elegance is always welcome back.',
                  textAlign: TextAlign.center,
                  style: TextStyle(
                    fontSize: 14,
                    height: 1.5,
...
extension TabBarMenuExtention on MainTabsState {
  ...
  void _initTabData(context) async {
    ...
    isInitialized = true;
    _lastTabSignature = _tabSignature();
  ...
  void _onTapTabBar(int index) {
    ...
    // Dashboard announcement popups targeted at this tab (home is handled by
    // the Home screen itself).
    final tabLayout = tabData?.layout;
    if (tabLayout != null && tabLayout != 'home') {
      PopupService.instance.maybeShowFor(tabLayout, context);
    }
